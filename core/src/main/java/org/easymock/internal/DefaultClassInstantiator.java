...
public class DefaultClassInstantiator implements IClassInstantiator {
    ...
    private RuntimeException runtimeException(Class<?> c, Exception e) {
        return new RuntimeException("Failed to instantiate " + classToPrint(c).getName() + "'s mock: ", e);
    }
    ...
    public Constructor<?> getConstructorToUse(Class<?> clazz) {
        ...
        try {
            ...
            if (clazz.getConstructors().length == 0) {
                throw new IllegalArgumentException("No visible constructors in class " + classToPrint(clazz).getName());
            }
            return clazz.getConstructors()[0];
        }
    }

    private Class<?> classToPrint(Class<?> clazz) {
        // If the class was generated by EasyMock, return the base class so the user knows which of his/her classes went wrong
        return clazz.getName().contains("$$$EasyMock$") ? clazz.getSuperclass() : clazz;
    }
